...
Keep the packaged UE4SS runtime on the LTS v3.0.1 line and make CMM validation
produce a real `VALIDATED` state only when that LTS runtime works against the
detected Clawed build.
...
3. If v3.0.1 loads but fails UE4SS signatures or object discovery, test only
   LTS-compatible fixes first: UE4SS settings, supported custom game config,
   mappings placement, and safe marker timing changes. Do not return to
   `experimental-latest` for the packaged default.
...
5. If the failure is a real unsupported LTS runtime limitation for the current
   Clawed build, keep Launch Modded non-blocking, keep the runtime
   `UNVALIDATED` or `INCOMPATIBLE` based on scoped evidence, and surface the
   evidence path clearly instead of overstating validation.
...
- The packaged runtime remains `ue4ss-v3.0.1-lts`.
