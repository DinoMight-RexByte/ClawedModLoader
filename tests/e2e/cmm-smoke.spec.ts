...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    ...
    const cmm: Record<string, any> = {
      ...
      getLogBundlePlan: async ({ mode, includeHardware }: any) => ({
        generatedAt: now,
        mode,
        fileName: `${mode === "modded" ? "Modded" : "Vanilla"}_ClawedLogs_24962487_aug-27-2026.zip`,
        ...
        sources: [
          ...
          },
          {
            ...
            scope: "vanilla",
            sourcePath: "C:\\Users\\Tester\\AppData\\Local\\Clawed\\Saved\\Logs",
          ...
          },
          {
            ...
            exists: true,
            included: includeHardware
          }
        ]
      }),
      chooseAndCreateLogBundle: async ({ mode, includeHardware }: any) => ({
        status: "created",
        bundlePath: `C:\\Bundles\\${mode === "modded" ? "Modded" : "Vanilla"}_ClawedLogs_24962487_aug-27-2026.zip`,
        fileName: `${mode === "modded" ? "Modded" : "Vanilla"}_ClawedLogs_24962487_aug-27-2026.zip`,
        steamBuildId: "24962487",
        fileCount: includeHardware ? 4 : 3,
        bytesWritten: 2048,
        includedHardware: includeHardware,
        problems: []
      }),
...
for (const viewport of responsiveViewports) {
  test(`renders primary pages at ${viewport.name} viewport`, async ({ page }) => {
    ...
    const pages = [
      ...
      { nav: "Log Bundler", heading: "Support Log Bundle" },
